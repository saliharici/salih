...
// Gemini API Ayarları (API Anahtarı config.js'den alındı)
...
async function initializeAppDisplay() {
     if (!firebaseConfig.apiKey) { document.body.innerHTML = "<h1>Firebase yapılandırması bulunamadı. Lütfen js/config.js dosyasını kontrol edin.</h1>"; return; }
     try {
        ...
        setLogLevel('error'); // Konsolu temiz tutmak için sadece hataları göster
        await signInAnonymously(auth); console.log("Firebase başlatıldı ve anonim kullanıcı girişi yapıldı.");
        ...
        // PERİYODİK ÇAĞRILAR
        setInterval(fetchWeather, 1800000); // 30 dakikada bir hava durumu
        setInterval(fetchFactOfTheDay, 6 * 60 * 60 * 1000); // 6 saatte bir günün bilgisi
        setInterval(fetchWordOfTheDay, 6 * 60 * 60 * 1000); // 6 saatte bir günün kelimesi
        ...
        document.getElementById("school-name").innerText = "Bağlantı Hatası!";
    }
}
...
function startRealtimeListener() {
    ...
    onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            console.log("Veri bulunamadı. Örnek veri oluşturuluyor...");
            createInitialData(docRef);
        }
    }, (error) => {
        console.error("Veri Okuma Hatası:", error);
        document.getElementById("school-name").innerText = "Veri Okuma Hatası!";
    });
}

 async function createInitialData(docRef) {
     const sampleData = {
        schoolName: "Okul Adı", logoUrl: "", scrollingText: "Okulumuza hoş geldiniz! Yönetim panelinden bu alanı düzenleyebilirsiniz.", scrollingTextSize: "24px", scrollingTextColor: "#CFD8DC",
        announcements: ["Örnek duyuru 1: Yönetim panelinden duyuruları güncelleyin.", "Örnek duyuru 2: Panonuz artık çalışıyor."],
        dutyList: { pazartesi: { "KANTİN": "Öğretmen A", "BAHÇE": "Öğretmen B" } },
        countdownEvent: { name: "Yaz Tatiline", date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 16) },
        slideshowImages: ["https://placehold.co/800x400/37474F/ECEFF1?text=Yönetim+Panelinden+Resim+Ekleyin"],
        examSuccess: { title: "Örnek Sınav Başarısı", grades: { "12": [{name:"Ayşe Yılmaz", score:"480"}, {name:"Ali Veli", score:"475"}] } },
        showFactOfTheDay: true,
        classSchedule: { pazartesi: { "9-A": ["Matematik","Matematik","Fizik","Fizik","Kimya","Kimya","Biyoloji","Biyoloji"] } },
        showCommonExams: false,
        commonExamTitle: "Ortak Sınav Takvimi",
        commonExamDates: ["01.11.2025, 9. Sınıflar, Matematik, 1. Ders, 40 Dakika", "02.11.2025, 10. Sınıflar, Fizik, 2. Ders, 40 Dakika"]
     };
    try {
        await setDoc(docRef, sampleData);
        console.log("Örnek veri başarıyla oluşturuldu.");
    } catch (error) {
        console.error("Örnek veri oluşturulurken hata:", error);
    }
}
...
function renderCommonExams(title, dates, show) {
    ...
    if (show && dates && dates.length > 0) {
        ...
        dates.forEach(item => {
            ...
            if (parts.length < 3) return; // Tarih, Sınıf, Ders minimum

            const date = (parts[0] || "").trim();
            const grade = (parts[1] || "").trim();
            const subject = (parts[2] || "").trim();
            const timeRaw = (parts[3] || "").trim();
            const duration = (parts[4] || "").trim();

            let time = timeRaw;
            if (timeRaw.toUpperCase().includes("DERS")) {
                const lessonIndex = parseInt(timeRaw.match(/\d+/)?.[0], 10);
                if (lessonIndex > 0 && lessonIndex <= lessonTimes.length) {
                     time = `${lessonTimes[lessonIndex - 1].start}`;
                }
            }
            ...
            li.innerHTML = `
                ...
                <span class="exam-time">${time} ${duration ? '/ ' + duration : ''}</span>
            `;
...
async function fetchWeather() { const url = `https://wttr.in/${encodeURIComponent(weatherLocation)}?format=j1`; try { const response = await fetch(url); if (!response.ok) throw new Error("Hava durumu verisi alınamadı."); const data = await response.json(); const current = data.current_condition[0]; const temp = current.temp_C; const desc = current.lang_tr[0].value; const weatherCode = current.weatherCode; document.getElementById("weather-icon").innerText = getWeatherIcon(weatherCode); document.getElementById("weather-temp").innerText = `${temp}°C`; document.getElementById("weather-desc").innerText = desc; renderForecast(data.weather); fetchWeatherAdvice(temp, desc); } catch (error) { console.error("Hava durumu hatası:", error); document.getElementById("weather-desc").innerText = "Hata oluştu."; } }
 ...
 async function callGeminiTextApi(prompt) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return "İçerik alınamadı (API Hatası).";
        }
        const result = await response.json();
        const text = result.candidates?.[0]?.content?.parts?.[0]?.text;
        if (text) { return text.trim().replace(/^"|"$/g, ''); }
        else {
            console.warn("Gemini'den boş yanıt geldi.", result);
            return "İçerik alınamadı.";
        }
    } catch (error) {
        console.error("Gemini API'ye bağlanırken ağ hatası:", error);
...
function goToPage(pageNumber) {
    ...
    for (let i = 1; i <= TOTAL_PAGES; i++) {
        document.getElementById(`page${i}`).classList.add('hidden-page');
    ...
    document.getElementById(`nav-btn-${pageNumber}`).classList.add('active');
